...
async function seed({ force=false }={}) {
  ...
  if (force) {
    ...
    await Promise.all([
      Region, District, School, User, Report, Payment, Budget,
      Allocation, Message, Notification, AuditLog, FAQ,
      ChatSession, PendingQuestion, require('../models/Disbursement'),
    ].map(M => M.deleteMany({})));
  }
  ...
  const users = [
    ...
    { _id:'usr-regaud', username:'regionalauditor', password_hash:h('regaud123'),  role:'regional_auditor',     name:'Mr. Kofi Asante-Regionaud',     title:'Regional Auditor, Western North',              region_id:WNR,  district_id:null,      school_id:null, active:true, created_at:nowISO() },
    { _id:'usr-regmon', username:'regionalmon',     password_hash:h('regmon123'),  role:'regional_monitoring',  name:'Ms. Ama Boateng-Monitor',       title:'Regional M&E Officer, Western North',          region_id:WNR,  district_id:null,      school_id:null, active:true, created_at:nowISO() },
    // ── District ─────────────────────────────────────────────────
    ...
    { _id:'usr-dce',    username:'dce1',             password_hash:h('dce123'),     role:'dce',                  name:'Hon. Kwame Owusu-Darko',        title:'District Chief Executive, Sefwi Akontombra',   region_id:WNR,  district_id:'dst-akt', school_id:null, active:true, created_at:nowISO() },
    ...
    { _id:'usr-mno',    username:'distmon1',        password_hash:h('distmon123'), role:'monitoring_officer',   name:'Mr. Seth Antwi-Monitor',        title:'District M&E Officer',                         region_id:WNR,  district_id:'dst-akt', school_id:null, active:true, created_at:nowISO() },
    // ── Headmasters ───────────────────────────────────────────────
    ...[1,2,3,4,5,6,7,8].map(i => ({
      _id:`usr-h00${i}`, username:`head${i}`, password_hash:h('head123'),
      role:'headmaster', name:`Headmaster ${i}`, title:null,
      region_id:WNR, district_id:'dst-akt', school_id:`sch-00${i}`,
      active:true, created_at:nowISO(),
    })),
    // ── Caterers ──────────────────────────────────────────────────
    ...[1,2,3,4,5,6,7,8].map(i => ({
      _id:`usr-c00${i}`, username:`caterer${i}`, password_hash:h('cat123'),
      role:'caterer', name:`Caterer ${i}`, title:null,
      region_id:WNR, district_id:'dst-akt', school_id:`sch-00${i}`,
      rate_per_student:2.00, active:true, created_at:nowISO(),
    })),
  ];
  ...
  const foods = [
    ...
    'Beans Stew with Gari','Tuo Zaafi with Ayoyo Soup',
  ];
  const reports = [];
  for (let d = 0; d < 30; d++) {
    for (let i = 1; i <= 8; i++) {
      ...
      reports.push({
        ...
        time_ready:`${10 + Math.floor(Math.random()*2)}:30`,
        time_served:`${12 + Math.floor(Math.random()*2)}:00`,
        ...
        submitted_at:daysAgoISO(d)+'T13:00:00.000Z',
      });
    }
  }
  ...
  const payments = schools.map((sc, i) => {
    const covered = 60 + i, paid = 35 + i*2, arrears = covered - paid, rate = 2.00;
    return {
      ...
      status:arrears === 0 ? 'fully-paid' : 'partial',
      last_payment_date:daysAgoISO(10 + i*3),
      source:'National Government - GSFP',
      reference:`GSFP-2025T1-${String(i+1).padStart(4,'0')}`,
  ...
  await AuditLog.create({
    _id:newId('aud'), timestamp:nowISO(), user_id:'system', user_name:'System', user_role:'system',
    action:'SYSTEM_INITIALIZED', target:'platform',
    details:'GSFP v2 seeded: 16 regions, 5 districts, 8 schools, 34 users', level:'info',
  });

  console.log('[seed] Done:', {
    regions:regions.length, districts:districts.length, schools:schools.length,
...
async function seedFAQs() {
  ...
  await FAQ.insertMany([
    ...
    { _id:'faq-003', question:'What happens if my report is rejected?',       answer:'If your report is rejected by the headmaster, you will see a red alert on your dashboard showing the rejection reason. You can immediately resubmit a corrected report for the same day. The old rejected report is archived automatically.',                                                                                                              category:'reports',  keywords:['rejected','resubmit','correction','headmaster','reject'],           usage_count:0, created_by:'system', active:true, created_at:now },
    { _id:'faq-004', question:'How do I change my password?',                 answer:'To change your password: 1) Click "Change Password" in the left sidebar under Account. 2) Enter your current password. 3) Enter and confirm your new password. 4) Click "Change password".',                                                                                                                                                            category:'account',  keywords:['change','password','reset','account','security'],                   usage_count:0, created_by:'system', active:true, created_at:now },
    { _id:'faq-005', question:'How do I upload a profile picture?',           answer:'Click "My Profile" in the sidebar, then click the camera icon on your avatar. Select an image (JPG, PNG or WebP, max 5MB) and click "Save profile". Your photo will appear in the sidebar immediately.',                                                                                                                                                category:'account',  keywords:['profile','picture','photo','avatar','upload'],                      usage_count:0, created_by:'system', active:true, created_at:now },
    { _id:'faq-006', question:'How are arrears calculated?',                  answer:'Arrears = Days Covered minus Days Paid. Example: 60 days covered, 45 days paid = 15 days arrears. In GHS: 15 × enrolled pupils × GHS 2.00 = total arrears owed.',                                                                                                                                                                                     category:'payments', keywords:['arrears','calculate','days','covered','paid','owed'],               usage_count:0, created_by:'system', active:true, created_at:now },
    { _id:'faq-007', question:'How do I export reports to PDF or Excel?',     answer:'On any reports or payments screen, look for the PDF and Excel buttons in the top right area. The export includes all visible records with your current filters applied.',                                                                                                                                                                               category:'reports',  keywords:['export','pdf','excel','download','print'],                          usage_count:0, created_by:'system', active:true, created_at:now },
    { _id:'faq-008', question:'How does bulk payment upload work?',           answer:'Go to Bulk Upload in the sidebar. Download the CSV template, fill in your bank data (school_code, period, days_covered, days_paid, payment_date), then upload the file. The system automatically calculates arrears at GHS 2.00 per pupil per day.',                                                                                                    category:'payments', keywords:['bulk','upload','csv','bank','payment','import'],                    usage_count:0, created_by:'system', active:true, created_at:now },
    { _id:'faq-009', question:'Who approves feeding reports?',                answer:'Chain: 1) Caterer submits. 2) Headmaster approves or rejects. 3) DCE approves before forwarding to region. 4) Regional Coordinator reviews. 5) Regional Minister approves. 6) National Director reviews. 7) CEO has final oversight.',                                                                                                                  category:'workflow', keywords:['approve','approval','chain','workflow','headmaster','district','regional'], usage_count:0, created_by:'system', active:true, created_at:now },
    { _id:'faq-010', question:'How do I send a message to another user?',     answer:'Go to Messages in the sidebar. Click Compose. Choose your recipient (a broadcast group or individual). Set priority, add subject and message, then click Send.',                                                                                                                                                                                       category:'messages', keywords:['send','message','compose','broadcast','communicate'],               usage_count:0, created_by:'system', active:true, created_at:now },
  ]);
