...
import { School, Users, FileText, CreditCard, TrendingUp, Clock, CheckCircle2, AlertTriangle, MapPin, BarChart3, Download, RefreshCw, Award, Activity, Target, Zap, UserCheck, Bell } from 'lucide-react';
...
import { fmtNum, cedis, fmtDate, fmtDateTime, ROLE_LABELS } from '../../utils/format';
...
export default function DistrictDashboard({ onNavigate }) {
  ...
  const [enrModal,  setEnrModal]= useState(null);
  const [enrComment,setEnrCmt]= useState('');
  ...
  const load = useCallback(()=>{
    Promise.allSettled([
      ...
      api.reports.list({limit:200}), api.payments.list(),
    ...
    fetch((import.meta.env.VITE_BACKEND_URL||'')+'/api/enrollment?status=pending',
      { headers:{ Authorization:'Bearer '+localStorage.getItem('gsfp.token') } })
      .then(r=>r.json()).then(d=>setEnrReqs(d.requests||[])).catch(()=>{});
  },[]);

  useEffect(()=>{ load(); const t=setInterval(load,90000); return()=>clearInterval(t); },[]);

  const doEnrReview = async (action) => {
    if (action==='rejected'&&!enrComment.trim()) { setErr('Rejection reason required'); return; }
    setBusy(true); setErr(null);
    try {
      await fetch((import.meta.env.VITE_BACKEND_URL||'')+`/api/enrollment/${enrModal._id||enrModal.id}/review`, {
        method:'POST', headers:{'Content-Type':'application/json','Authorization':'Bearer '+localStorage.getItem('gsfp.token')},
        body:JSON.stringify({ action, comment:enrComment }),
      });
      setOk(`Enrollment update ${action}. ${action==='approved'?'School enrollment has been updated!':''}`);
      setEnrModal(null); setEnrCmt(''); load();
  ...
  const schoolPerf    = schools.map(s=>{
    ...
    return {...s,reports:sr.length,approved:ap,rate:sr.length>0?Math.round(ap/sr.length*100):0};
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="relative overflow-hidden rounded-2xl p-5" style={{background:'linear-gradient(135deg,#15493B 0%,#0f3329 100%)'}}>
        ...
        <div className="relative z-10 mt-3 space-y-2">
          {missingToday.length>0&&(
            <div className="flex items-center gap-2 bg-rust/20 border border-rust/30 rounded-xl px-3 py-2">
          ...
          {enrReqs.length>0&&(
            <div className="flex items-center gap-2 bg-blue-500/20 border border-blue-400/30 rounded-xl px-3 py-2 cursor-pointer" onClick={()=>{}}>
              <Bell className="w-4 h-4 text-blue-300 flex-shrink-0"/>
              <span className="text-sm text-blue-200 font-medium">{enrReqs.length} enrollment update{enrReqs.length!==1?'s':''} awaiting your approval</span>
            </div>
          )}
        </div>
      </div>
      ...
      {enrReqs.length>0&&(
        <Card>
          <h3 className="font-semibold text-ink mb-4 flex items-center gap-2"><UserCheck className="w-4 h-4 text-navy"/>Enrollment Update Requests — Pending Your Approval ({enrReqs.length})</h3>
          <div className="space-y-3">
            {enrReqs.map(r=>(
              <div key={r._id||r.id} className="p-4 bg-blue-50 border-2 border-blue-200 rounded-xl">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <div className="font-semibold text-ink">{r.school?.name||r.school_id}</div>
                    <div className="text-sm text-stone-500 mt-0.5">By: {r.headmaster_name} · {fmtDateTime(r.created_at)}</div>
                    <div className="flex items-center gap-2 mt-1.5">
                      <span className="text-sm text-stone-600">Current: <strong>{fmtNum(r.current_enrolled)}</strong></span>
                      <span className="text-stone-400">→</span>
                      <span className={`text-sm font-bold ${r.requested_enrolled>r.current_enrolled?'text-emerald':'text-amber'}`}>{fmtNum(r.requested_enrolled)} pupils</span>
                      <Pill tone="stone">{r.change_type}</Pill>
                    </div>
                    <div className="text-xs text-stone-500 mt-1 italic">"{r.reason}"</div>
                  </div>
                  <button onClick={()=>{ setEnrModal(r); setEnrCmt(''); setErr(null); }}
                    className="px-3 py-1.5 bg-navy text-white text-xs rounded-xl font-semibold hover:bg-navy/90 flex-shrink-0">
                    Review
                  </button>
                </div>
              </div>
            ...
            <div className="space-y-2.5">
              {caterers.slice(0,5).map((cat,i)=>(
                <div key={i} className="flex items-center gap-3 py-1.5 border-b border-stone-50 last:border-0">
                  ...
                  <div className="flex-1 min-w-0"><div className="text-sm font-medium text-ink truncate">{cat.name||'—'}</div></div>
          ...
          {schools.map(s=>{
            ...
            return (
              <div key={s._id||s.id} onClick={()=>setSel(s)}
                ...
                {rep?<div className={`mt-1.5 text-xs font-medium ${isApproved?'text-emerald':'text-amber'}`}>{isApproved?`✓ ${fmtNum(rep.students_fed)} fed`:'⏳ Pending'}</div>
              ...
              {schoolPerf.map((s,i)=>{
                ...
                return (
                  <tr key={s._id||s.id} className="hover:bg-paper cursor-pointer" onClick={()=>setSel(s)}>
                    ...
                    <td className="px-4 py-3"><div className="flex items-center gap-2 justify-center"><div className="w-16 h-1.5 bg-stone-100 rounded-full"><div className={`h-full bg-${tone} rounded-full`} style={{width:`${s.rate}%`}}/></div><span className={`text-xs font-bold text-${tone}`}>{s.rate}%</span></div></td>
                    <td className="px-4 py-3 text-center">{rep?<Pill tone={rep.status==='approved'?'emerald':'amber'}>{rep.status==='approved'?'✓':'⏳'}</Pill>:<Pill tone="rust">Missing</Pill>}</td>
                  </tr>
                );
              })}
            </tbody>
        ...
        {selSch&&(
          <div className="space-y-4">
            ...
            <div className="space-y-2 max-h-52 overflow-y-auto">
              {selReports.length===0?<p className="text-stone-300 text-sm text-center py-4">No reports</p>
              :selReports.map(r=>(
                <div key={r._id||r.id} className={`flex items-center justify-between p-3 rounded-xl border ${r.status==='approved'?'border-emerald/20 bg-emerald/5':r.status==='rejected'?'border-rust/20 bg-rust/5':'border-amber/20 bg-amber/5'}`}>
                  <div><div className="text-sm font-medium text-ink">{fmtDate(r.date)} · {r.food_type}</div><div className="text-xs text-stone-400">{fmtNum(r.students_fed)} pupils fed</div></div>
      ...
      <Modal open={!!enrModal} onClose={()=>setEnrModal(null)} title="Review Enrollment Update Request" size="md">
        {err&&<div className="mb-3 text-sm text-rust bg-rust/10 rounded-lg p-2">{err}</div>}
        {enrModal&&(
          <div className="space-y-4">
            <div className="bg-stone-50 rounded-xl p-4">
              <div className="font-semibold text-ink">{enrModal.school?.name||enrModal.school_id}</div>
              <div className="text-sm text-stone-500 mt-0.5">Submitted by: {enrModal.headmaster_name} · {fmtDateTime(enrModal.created_at)}</div>
              <div className="flex items-center gap-3 mt-3">
                <div className="text-center bg-white rounded-xl p-3 border">
                  <div className="text-xs text-stone-400">Current</div>
                  <div className="text-xl font-bold text-ink">{fmtNum(enrModal.current_enrolled)}</div>
                </div>
                <div className="text-stone-400 text-xl">→</div>
                <div className="text-center bg-white rounded-xl p-3 border">
                  <div className="text-xs text-stone-400">Requested</div>
                  <div className={`text-xl font-bold ${enrModal.requested_enrolled>enrModal.current_enrolled?'text-emerald':'text-amber'}`}>{fmtNum(enrModal.requested_enrolled)}</div>
                </div>
                <Pill tone="stone">{enrModal.change_type}</Pill>
              </div>
              <div className="mt-3 text-sm text-stone-600 italic">"{enrModal.reason}"</div>
            </div>
            <div className="bg-amber/10 border border-amber/20 rounded-xl p-3 text-xs text-amber">
              <strong>Note:</strong> If approved, the school's enrollment will be updated to <strong>{fmtNum(enrModal.requested_enrolled)}</strong> pupils. This becomes the new maximum for caterer reports.
            </div>
            <Textarea label="Comment" value={enrComment} onChange={e=>setEnrCmt(e.target.value)} rows={2} placeholder="Add your comments or rejection reason..."/>
            <div className="grid grid-cols-2 gap-3">
              <button onClick={()=>doEnrReview('approved')} disabled={busy}
                className="py-3 bg-emerald text-white rounded-xl font-bold hover:bg-emerald/90 disabled:opacity-50">
                ✓ Approve & Update Enrollment
              </button>
              <button onClick={()=>doEnrReview('rejected')} disabled={busy}
                className="py-3 bg-rust text-white rounded-xl font-bold hover:bg-rust/90 disabled:opacity-50">
                ✗ Reject Request
              </button>
            </div>
          </div>
