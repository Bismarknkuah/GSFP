...
export default function MonitoringOfficerPanel() {
  ...
  const [menuConfs, setMenuConfs] = useState([]);
  ...
  const load = useCallback(async()=>{
    ...
    fetch(`${BASE}/api/timetable/confirmations?date=${day}`, { headers:authH })
      .then(x=>x.ok?x.json():{confirmations:[]}).then(d=>setMenuConfs(d.confirmations||[])).catch(()=>{});
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Live school grid */}
      <Card>
        <h3 className="font-semibold text-ink mb-3">🍲 Menu Compliance Today</h3>
        {menuConfs.length===0 ? (
          <p className="text-xs text-stone-400 mb-4">No headmaster menu confirmations yet today.</p>
        ) : (
          <div className="grid sm:grid-cols-2 gap-2 mb-5">
            {menuConfs.map(c=>{
              const sch = schools.find(s=>(s._id||s.id)===c.school_id);
              return (
                <div key={c._id} className={`p-2.5 rounded-xl border text-xs ${c.matched?'border-emerald/20 bg-emerald/5':'border-amber/30 bg-amber/5'}`}>
                  <div className="font-semibold text-ink">{sch?.name||c.school_id}</div>
                  {c.matched
                    ? <span className="text-emerald">✓ Served as scheduled: {c.scheduled_food||'—'}</span>
                    : <span className="text-amber">⚠ Scheduled "{c.scheduled_food||'—'}" but served "{c.served_food}"</span>}
                </div>
              );
            })}
          </div>
