export const fmtDate     = (iso) => { if(!iso)return'--'; try{return new Date(iso.slice(0,10)+'T00:00:00').toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'});}catch{return iso;} };
export const fmtDateTime = (iso) => { if(!iso)return'--'; try{return new Date(iso).toLocaleString('en-GB',{day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'});}catch{return iso;} };
export const fmtNum      = (n)   => { const x=Number(n); return Number.isFinite(x)?x.toLocaleString('en-GH'):'0'; };
export const cedis       = (n)   => { const x=Number(n); return Number.isFinite(x)?'GHS '+x.toLocaleString('en-GH',{minimumFractionDigits:2,maximumFractionDigits:2}):'GHS 0.00'; };
...
export const ROLE_TIER = (role) => {
  if (['ceo','national_director','super_admin','national_admin','national_finance','national_auditor','national_monitoring'].includes(role)) return 'national';
  if ((role||'').startsWith('regional_') || role==='regional_minister') return 'regional';
  if (['district_director','dce','district_coordinator','district_admin','finance_officer','auditor','monitoring_officer','data_entry'].includes(role)) return 'district';
  return 'school';
};
